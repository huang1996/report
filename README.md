...
| `-ident_engineer_tail` | ident 末尾段判定：`auto`（默认，摘掉它后仍能解析出角色才当工程师）/ `always`（末尾 2~4 汉字一律当工程师）/ `never`（不识别工程师） | `auto` |
...
## 主机角色 / 归属的解析规则（`-layout`、`-ident_engineer_tail`）
...
`-ident_engineer_tail` 决定末尾段是「角色」还是「运维工程师」——两者都可能是 2~4 个纯汉字
（`电子签章` 是角色、`邹源` 是姓名），字面无法区分。程序按**角色必备、工程师可选**的约定判定：

| 取值 | 行为 |
|---|---|
| `auto`（默认） | 只有当摘掉末尾段后**仍能解析出角色**时，才认定它是工程师姓名；否则把它当作角色本身 |
| `always` | 沿用旧行为：末尾 2~4 个纯汉字一律视为工程师（可能把「电子签章」这类纯中文角色错当成人名，导致角色为空） |
| `never` | 不做工程师识别，末尾段一律并入角色 |

例如 `000002-10.194.67.194-泸州市环保三级统筹项目-电子签章`：`auto` 下解析为
项目=`泸州市环保三级统筹项目`、角色=`电子签章`；`always` 下则会变成角色为空、工程师=`电子签章`。

> 注：报告信息表的「运维工程师」由 `-report_engineer` 统一覆盖，`ident` 里的工程师仅在未指定时生效。
